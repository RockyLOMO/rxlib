...
//    }
    ...
    public MonitorBean getBean() {
        ...
        return new MonitorBean(os.getAvailableProcessors(), cpuLoad, thread.getThreadCount(),
                ...
                Cache.getOrSet(cacheKey("getBean"), k -> NQuery.of(File.listRoots()).select(p -> new DiskMonitorBean(p.getPath(), p.getTotalSpace() - p.getFreeSpace(), p.getTotalSpace())), Cache.WEAK_CACHE));
    }
}
