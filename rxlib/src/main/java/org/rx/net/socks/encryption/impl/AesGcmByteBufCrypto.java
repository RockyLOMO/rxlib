...
abstract class AesGcmByteBufCrypto implements ICrypto, AutoCloseable {
    ...
    private static final FastThreadLocal<byte[]> SALT_BUF = new FastThreadLocal<byte[]>() {
        ...
        protected byte[] initialValue() {
            return new byte[MAX_SALT_LENGTH];
        }
    };
    ...
    private static final FastThreadLocal<byte[]> UDP_SUBKEY = new FastThreadLocal<byte[]>() {
        @Override
        protected byte[] initialValue() {
            return new byte[MAX_KEY_LENGTH];
        }
    };
    ...
    AesGcmByteBufCrypto(String name, String cipherName, String password, int keyLength, int saltLength, boolean directBuf) {
        if (!cipherName.equalsIgnoreCase(name)) {
            throw new IllegalArgumentException("Unsupported cipher: " + name);
        }
        this.cipherName = cipherName;
        this.keyLength = keyLength;
        this.saltLength = saltLength;
        this.ssKey = new ShadowSocksKey(password, keyLength);
    ...
    public ByteBuf encrypt(ByteBuf in) {
        ...
        int estimatedSize = forUdp
                ? saltLength + readable + TAG_LENGTH
                : (encCipher == null ? saltLength : 0) + readable + chunks * (LEN_TAG_SIZE + TAG_LENGTH);
    ...
    public ByteBuf decrypt(ByteBuf in) {
        ...
        int estimatedSize;
        if (forUdp) {
            estimatedSize = Math.max(0, in.readableBytes() - saltLength - TAG_LENGTH);
        } else {
            estimatedSize = decCipher == null
                    ? Math.max(0, in.readableBytes() - saltLength)
                    : in.readableBytes();
        }
        ByteBuf out = allocate(estimatedSize);
        try {
    ...
    private void encryptTcp(ByteBuf in, ByteBuf out) {
        try {
            if (encCipher == null) {
                ...
                out.writeBytes(salt, 0, saltLength);
                encSubkey = new byte[keyLength];
            ...
            throw new DecoderException(cipherName + " TCP encrypt failed", e);
        }
    }
    ...
    private void decryptTcp(ByteBuf in, ByteBuf out) {
        try {
            ...
            throw new DecoderException(cipherName + " TCP decrypt failed", e);
        }
    }

    private boolean readTcpSalt(ByteBuf in) throws GeneralSecurityException {
        byte[] salt = SALT_BUF.get();
        if (saltBytesRead == 0 && in.readableBytes() >= saltLength) {
            in.readBytes(salt, 0, saltLength);
        } else {
            int readNow = Math.min(saltLength - saltBytesRead, in.readableBytes());
            if (readNow <= 0) {
                return false;
            }
            ByteBuf pending = pendingEncrypted();
            ...
            if (saltBytesRead < saltLength) {
                return false;
            }
            pending.getBytes(0, salt, 0, saltLength);
            pending.clear();
            saltBytesRead = 0;
        }
        decSubkey = new byte[keyLength];
    ...
    private void encryptUdp(ByteBuf in, ByteBuf out) {
        try {
            ...
            throw new DecoderException(cipherName + " UDP encrypt failed", e);
        }
    }

    private void decryptUdp(ByteBuf in, ByteBuf out) {
        try {
            if (in.readableBytes() < saltLength + TAG_LENGTH) {
            ...
            throw new DecoderException(cipherName + " UDP decrypt failed", e);
        }
    }
    ...
    private void initCipher(Cipher cipher, int mode, byte[] key, byte[] nonce) throws GeneralSecurityException {
        cipher.init(mode, new SecretKeySpec(key, 0, keyLength, AES), new GCMParameterSpec(TAG_BITS, nonce));
    }

    private void genSubkey(byte[] salt, byte[] okm) throws GeneralSecurityException {
        Mac mac = HKDF_MAC.get();
        byte[] prk = HKDF_PRK.get();
        mac.init(new SecretKeySpec(salt, 0, saltLength, HKDF_ALGORITHM));
        mac.update(ssKey.getEncoded());
        mac.doFinal(prk, 0);

        byte[] block = HKDF_BLOCK.get();
        Arrays.fill(okm, 0, keyLength, (byte) 0);
        int copied = 0;
        int counter = 1;
        int blockLen = 0;
        while (copied < keyLength) {
            ...
            int copy = Math.min(blockLen, keyLength - copied);
