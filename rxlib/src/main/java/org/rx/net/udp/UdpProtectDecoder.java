...
/**
 * UDP Protect 入站解码器。先去重，再进入 FEC 分组，避免多倍副本污染恢复状态。
 */
@Slf4j
public class UdpProtectDecoder extends ChannelInboundHandlerAdapter {
    ...
    public UdpProtectDecoder(UdpProtectConfig config) {
        if (config == null) {
    ...
    private void decodeProtected(ChannelHandlerContext ctx, DatagramPacket packet) {
        ByteBuf content = packet.content();
        ...
        int flags = UdpProtectHeader.flags(content, readerIndex);
        int headerLen = UdpProtectHeader.headerLength(content, readerIndex);
        int codec = UdpProtectHeader.codec(content, readerIndex);
        int shardK = UdpProtectHeader.shardK(content, readerIndex);
        int shardP = UdpProtectHeader.shardP(content, readerIndex);
        int shardIdx = UdpProtectHeader.shardIdx(content, readerIndex);
        int sessionId = UdpProtectHeader.sessionId(content, readerIndex);
        int seq = UdpProtectHeader.seq(content, readerIndex);
        int groupId = UdpProtectHeader.groupId(content, readerIndex);
        int payloadLen = UdpProtectHeader.payloadLen(content, readerIndex);
    ...
    private boolean validBaseHeader(ByteBuf content, int readerIndex, int readable) {
        if (readable < UdpProtectHeader.HEADER_SIZE) {
            return false;
        }
        if (UdpProtectHeader.version(content, readerIndex) != UdpProtectHeader.VERSION) {
            return false;
        }
        int headerLen = UdpProtectHeader.headerLength(content, readerIndex);
        return headerLen >= UdpProtectHeader.HEADER_SIZE && readable >= headerLen;
    }

    private boolean validPacket(int flags, int codec, int shardK, int shardP, int shardIdx,
                                int payloadLen, int payloadBytes) {
        boolean data = (flags & UdpProtectHeader.FLAG_DATA) != 0;
        boolean parity = (flags & UdpProtectHeader.FLAG_PARITY) != 0;
        if (data == parity) {
            return false;
        }
        if (parity && codec != UdpProtectHeader.CODEC_XOR) {
            return false;
        }
        if (codec != UdpProtectHeader.CODEC_NONE && codec != UdpProtectHeader.CODEC_XOR) {
            return false;
        }
        if (codec == UdpProtectHeader.CODEC_XOR && (shardK <= 0 || shardK > config.getFecDataShards() || shardP != 1)) {
            return false;
        }
        if (data) {
            return shardIdx >= 0 && shardIdx < Math.max(1, shardK)
                    && payloadLen <= payloadBytes && payloadLen <= config.getMaxProtectedPayload();
        }
    ...
    private void handleData(ChannelHandlerContext ctx, DatagramPacket packet, ByteBuf content,
        ...
        if (!config.isFecEnabled() || codec != UdpProtectHeader.CODEC_XOR) {
            return;
        }
    ...
    private void tryRecoverOrComplete(ChannelHandlerContext ctx, DatagramPacket packet,
        ...
        ByteBuf recovered = group.tryRecover(ctx.alloc(), config.getMaxProtectedPayload());
        if (recovered != null) {
