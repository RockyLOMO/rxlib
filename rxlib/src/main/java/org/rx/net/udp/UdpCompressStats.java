...
public class UdpCompressStats {
    ...
    public boolean shouldBypass(InetSocketAddress recipient) {
        ...
        RouteState state = routeStates.get(UdpResilienceAttributes.normalize(recipient));
        if (state == null) {
            return false;
        }
    ...
    public void recordApplied(InetSocketAddress recipient) {
        ...
        RouteState state = routeStates.computeIfAbsent(UdpResilienceAttributes.normalize(recipient), k -> new RouteState());
