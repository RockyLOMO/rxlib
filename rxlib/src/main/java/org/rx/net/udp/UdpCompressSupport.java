 ...
 * UDP 压缩公共常量与 LZ4 工具。
 */
public final class UdpCompressSupport {
    public static final int HEADER_MAGIC = 0x55434D50; // "UCMP"
    public static final int HEADER_SIZE = 8;
    public static final short FLAG_DICT = 0x01;
    public static final int MAX_COMPRESSION_LEVEL = UdpCompressConfig.MAX_COMPRESSION_LEVEL;
    ...
    public static int maxCompressedLength(int srcLen, int compressionLevel) {
        return compressor(compressionLevel).maxCompressedLength(srcLen);
    }

    public static int compress(ByteBuf src, int srcIndex, int srcLen, ByteBuf dst, int dstIndex, int maxDstLen, int compressionLevel) {
    ...
    public static int decompress(ByteBuf src, int srcIndex, int srcLen, ByteBuf dst, int dstIndex, int maxDstLen) {
        BufferRef srcCopy = new BufferRef();
        try {
            ByteBuffer srcBuffer = nioReadableBuffer(src, srcIndex, srcLen, srcCopy);
            ByteBuffer dstBuffer = dst.nioBuffer(dstIndex, maxDstLen);
