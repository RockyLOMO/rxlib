...
/**
 * UDP Protect 配置。默认值面向低延迟游戏 UDP：XOR 3:1 FEC，冗余倍率默认 1。
 */
@Getter
@Setter
@ToString
public class UdpProtectConfig implements Serializable {
    ...
    private int maxProtectedPayload = 1200;
    ...
    private UdpProtectPolicy policy = UdpProtectPolicy.NONE;
    private UdpProtectFlowIdResolver flowIdResolver = UdpProtectFlowIdResolver.RECIPIENT;
    private transient UdpProtectStats stats = new UdpProtectStats();

    public static UdpProtectConfig gameLowLatency() {
        return new UdpProtectConfig();
    }

    public static UdpProtectConfig light() {
        UdpProtectConfig config = new UdpProtectConfig();
        config.setFecDataShards(4);
        config.setRedundantMultiplier(1);
        return config;
    }

    public static UdpProtectConfig extremeLoss() {
        UdpProtectConfig config = new UdpProtectConfig();
    ...
    public UdpProtectStats stats() {
        if (stats == null) {
            stats = new UdpProtectStats();
        }
        return stats;
    }
    ...
    public void setMaxProtectedPayload(int maxProtectedPayload) {
        this.maxProtectedPayload = Math.max(1, Math.min(0xFFFF, maxProtectedPayload));
    }
    ...
    public void setPolicy(UdpProtectPolicy policy) {
        this.policy = policy != null ? policy : UdpProtectPolicy.NONE;
    }

    public void setFlowIdResolver(UdpProtectFlowIdResolver flowIdResolver) {
        this.flowIdResolver = flowIdResolver != null ? flowIdResolver : UdpProtectFlowIdResolver.RECIPIENT;
    }

    public void setStats(UdpProtectStats stats) {
        this.stats = stats != null ? stats : new UdpProtectStats();
    }
}
