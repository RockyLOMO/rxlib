...
/**
 * UDP Resilience 出站编码器。不可 Sharable，每个 channel 持有独立 peer/FEC 状态。
 */
@Slf4j
public class UdpResilienceEncoder extends ChannelDuplexHandler {
    ...
    public UdpResilienceEncoder(UdpResilienceConfig config) {
        if (config == null) {
    ...
    public void write(ChannelHandlerContext ctx, Object msg, ChannelPromise promise) throws Exception {
        ...
        if (!config.isEnabled()
                || !UdpResilienceAttributes.shouldApply(ctx.channel(), recipient, config.isResilienceAll())
                || !shouldApplyPayload()) {
            super.write(ctx, msg, promise);
            return;
        }
        ...
        if (payloadLen > config.getMaxResiliencePayload()) {
    ...
    private boolean shouldApplyPayload() {
    ...
    private void writeFecData(ChannelHandlerContext ctx, DatagramPacket packet, PeerState state,
        ...
        ByteBuf resilienceBuf = encodePacket(ctx, UdpResilienceHeader.FLAG_DATA, UdpResilienceHeader.CODEC_XOR,
                group.capacity(), 1, shardIdx, flowId, seq, group.groupId,
                payloadLen, content, payloadIndex, payloadLen);
        writeResilience(ctx, resilienceBuf, packet.recipient(), effectiveDataMultiplier(), promise);
        stats.recordResilienceData();
    ...
    private void writeDataOnly(ChannelHandlerContext ctx, DatagramPacket packet, PeerState state,
        ...
        int payloadLen = content.readableBytes();
        int seq = state.nextSeq++;
        ByteBuf resilienceBuf = encodePacket(ctx, UdpResilienceHeader.FLAG_DATA, UdpResilienceHeader.CODEC_NONE,
    ...
    private void emitParity(ChannelHandlerContext ctx, PeerState state, int flowId) {
        ...
        try {
            ...
            int seq = state.nextSeq++;
            ByteBuf resilienceBuf = encodePacket(ctx, UdpResilienceHeader.FLAG_PARITY, UdpResilienceHeader.CODEC_XOR,
                    group.count(), 1, group.count(), flowId, seq, group.groupId,
                    parityLen, parity, parity.readerIndex(), parityLen);
            writeResilience(ctx, resilienceBuf, state.recipient, effectiveParityMultiplier(), ctx.voidPromise());
            stats.recordParity();
        } finally {
            parity.release();
        }
    ...
    private ByteBuf encodePacket(ChannelHandlerContext ctx, int flags, int codec, int shardK, int shardP,
        ...
        ByteBuf out = ctx.alloc().directBuffer(UdpResilienceHeader.HEADER_SIZE + payloadBytes);
        try {
            UdpResilienceHeader.write(out, flags, codec, shardK, shardP, shardIdx, flowId, seq, groupId, payloadLen);
    ...
    private void writeResilience(ChannelHandlerContext ctx, ByteBuf packetBuf, InetSocketAddress recipient,
    ...
    private static final class PeerKey {
        ...
        PeerKey(InetSocketAddress recipient, int flowId) {
            this.recipient = UdpResilienceAttributes.normalize(recipient);
            this.flowId = flowId;
        }
