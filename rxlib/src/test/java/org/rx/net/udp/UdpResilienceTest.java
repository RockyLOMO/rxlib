...
public class UdpResilienceTest {
    ...
    public void testEncoderEmitsXorParityOnFullGroup() {
        UdpResilienceConfig config = config(3);
        EmbeddedChannel channel = new EmbeddedChannel(new UdpResilienceEncoder(config));
        ...
        try {
            ...
            for (int i = 0; i < out.size(); i++) {
                ...
                assertEquals(UdpResilienceHeader.MAGIC, content.getUnsignedShort(content.readerIndex()));
            }
            ByteBuf parity = out.get(3).content();
            int readerIndex = parity.readerIndex();
            assertEquals(UdpResilienceHeader.FLAG_PARITY, UdpResilienceHeader.flags(parity, readerIndex));
            assertEquals(3, UdpResilienceHeader.shardK(parity, readerIndex));
            assertEquals(3, UdpResilienceHeader.shardIdx(parity, readerIndex));
    ...
    public void testDecoderRecoversVariableLengthPayload() {
        ...
        EmbeddedChannel decoder = new EmbeddedChannel(new UdpResilienceDecoder(config));
        try {
    ...
    public void testEncoderKeepsPeerGroupsIsolated() {
        UdpResilienceConfig config = config(2);
        EmbeddedChannel channel = new EmbeddedChannel(new UdpResilienceEncoder(config));
        ...
        try {
            ...
            for (DatagramPacket packet : out) {
                ...
                int flags = UdpResilienceHeader.flags(content, content.readerIndex());
                if ((flags & UdpResilienceHeader.FLAG_PARITY) != 0) {
    ...
    public void testRedundantCopiesDeliveredOnce() {
        ...
        EmbeddedChannel encoder = new EmbeddedChannel(new UdpResilienceEncoder(config));
        EmbeddedChannel decoder = new EmbeddedChannel(new UdpResilienceDecoder(config));
        try {
    ...
    @Test
    public void testResiliencePeerAttributeRestrictsEncoder() {
        UdpResilienceConfig config = config(3);
        ...
        assertNotNull(plain);
        assertFalse(UdpResilienceHeader.isResilienceFrame(plain.content()));
        plain.release();

        UdpResilienceAttributes.addResiliencePeer(channel, REMOTE_A);
        channel.writeOutbound(datagram("secure", REMOTE_A));
        DatagramPacket resiliencePacket = channel.readOutbound();
        assertNotNull(resiliencePacket);
        assertTrue(UdpResilienceHeader.isResilienceFrame(resiliencePacket.content()));
        resiliencePacket.release();
    }

    @Test
    public void testRealDatagramPipelineRecoversDroppedShard() throws Exception {
        UdpResilienceConfig senderConfig = config(3);
        UdpResilienceConfig receiverConfig = config(3);
        ...
        try {
            ...
            rb.group(group)
                    ...
                    .handler(new ChannelInitializer<DatagramChannel>() {
                        ...
                        protected void initChannel(DatagramChannel ch) {
                            ...
                            ch.pipeline().addLast(new UdpResilienceDecoder(receiverConfig));
            ...
            sb.group(group)
                    ...
                    .handler(new ChannelInitializer<DatagramChannel>() {
                        @Override
                        protected void initChannel(DatagramChannel ch) {
                            ch.pipeline().addLast(new UdpResilienceEncoder(senderConfig));
                        }
                    });
    ...
    private static UdpResilienceConfig config(int dataShards) {
        UdpResilienceConfig config = UdpResilienceConfig.gameLowLatency();
    ...
    private static List<DatagramPacket> encode(UdpResilienceConfig config, InetSocketAddress remote, String a, String b, String c) {
        EmbeddedChannel encoder = new EmbeddedChannel(new UdpResilienceEncoder(config));
        ...
        public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
            if (msg instanceof DatagramPacket) {
                ...
                if (UdpResilienceHeader.isResilienceFrame(content)
                        && (UdpResilienceHeader.flags(content, readerIndex) & UdpResilienceHeader.FLAG_DATA) != 0
                        && UdpResilienceHeader.shardIdx(content, readerIndex) == shardIdx) {
