...
```java
static final Map<HybridSession, Map<Integer, ClientBean>> clientBeans = new ConcurrentHashMap<>();
static final Map<HybridSession, AtomicInteger> clientRefCounts = new ConcurrentHashMap<>();
```
...
MethodMessage/EventMessage/MetadataMessage
  -> HybridClient.session().send(pack, options)
  -> HybridRoutePolicy 自动选择 TCP 或 UDP
```

默认路由建议：

- `MetadataMessage`：强制 TCP。
- `EventMessage.SUBSCRIBE/UNSUBSCRIBE/COMPUTE_ARGS`：强制 TCP。
- `MethodMessage`：默认自动路由，小包可 UDP，严格有序场景可强制 TCP。
- `MethodMessage` 响应：默认自动路由，异常响应和大响应走 TCP。
- 事件广播：默认自动路由，要求严格顺序的事件强制 TCP。
...
- `clientBeans` key 从 `DefaultTcpClient` 改为 `HybridSession`。
- `clientRefCounts` key 从 `DefaultTcpClient` 改为 `HybridSession`。
- `getClientBeans(...)`、`retainClient(...)`、`releaseClient(...)` 改为接收 `HybridSession`。
...
```java
private static void init(HybridClient client, HybridSession session, Object proxyObject, FastThreadLocal<Boolean> isCompute)
```
...
- `session.onReceive` 处理 `EventMessage` 与 `MethodMessage` 响应。
- `MetadataMessage` 使用 `session.send(..., HybridSendOptions.FORCE_TCP)`。
...
- 待 `onSessionReady` 后，遍历旧 session pending beans，迁移到新 session wait map。
- pending pack 通过新 session 强制 TCP 重发，保证 hello/metadata 之后有序。
- 老 session detach 后清空 wait map，未迁移成功的请求按断链失败。
...
final class RemotingHybridOptions {
    static final HybridSendOptions CONTROL = HybridSendOptions.forceTcp();
    static final HybridSendOptions METHOD = new HybridSendOptions();
    static final HybridSendOptions RESPONSE = new HybridSendOptions();
...
- 控制消息强制 TCP。
- 普通 method request/response 自动路由。
- 事件订阅控制强制 TCP。
- 事件广播自动路由。
...
- `clientBeans` 按 `HybridSession` 存储，session close 时一次性 remove。
...
- `clientBeans/clientRefCounts/event subscribe` 的 key 替换为 `HybridSession`。
- 编译通过。
...
- 小 MethodMessage 可走 UDP。
- 大 MethodMessage 走 TCP。
- 控制消息强制 TCP。
- UDP ACK timeout 后 TCP 补发，业务只投递一次。
- TCP reconnect 后重新 metadata 并重发 pending。
...
Remoting 可以迁移到 Hybrid，但不能直接把 `DefaultTcpClient.send(...)` 简单替换成 `HybridClient.send(...)`。必须先让 Hybrid 服务端接收事件携带 session，并让 session 具备 attr、生命周期和池化复用能力。完成这些前置能力后，Remoting 内部可整体以 `HybridSession` 为连接身份，方法调用和事件模型不需要依赖 TCP channel。
